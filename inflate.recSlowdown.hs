...
data DeflateState = DeflateState {strIn :: BitStream, out :: OutChars Word8, inLen :: Int }
...
throwUp aleph = 
  do (bitn, byte, _) <- gets strIn
     sofar <- gets out
     DeflateParse {unDeflateParse = throwError (aleph ++ "(at bit=" ++ (show bitn) ++ ", byte=" ++ (show byte) ++ ", decompressed so far:" ++ (show $ Data.ByteString.pack (reverse $ oc2list sofar)) ++ ")")}

--throwUp aleph = error aleph

peekBit :: BitStream -> Maybe Bool
peekBit (_, _, []) = Nothing
peekBit (i, _, x) = Just $ testBit (head x) i

popBit :: BitStream -> Maybe BitStream
popBit (7, _, []) = Nothing
popBit (7, j, _ : l) = Just (0, j+1, l)
popBit (i, j, x) = Just ((i + 1), j, x)
...
finishByte bs =
  case bs of
    (0, _, _)  -> Just bs
    (_, _, []) -> Nothing
    (_, j, (_ : l)) -> Just (0, j+1, l)

peekByte :: BitStream -> Either String Word8
peekByte (_, _, []) = Left "peekByte at end of stream"
peekByte (i, j, (x : _)) = if (i == 0) then Right x
                           else Left ("peekByte not at byte position in byte " ++ (show j))
...
mapOut :: (OutChars Word8 -> OutChars Word8) -> DeflateState -> DeflateState
mapOut f st = st {out = f (out st)}

setOut :: OutChars Word8 -> DeflateState -> DeflateState
...
uncompressedBlock = do
  ...
  if (and (map (\ (y, z) -> not (y == z)) (zip lenBits negLenBits)))
    then do !bytes <- parseBytesReverse (lsbToInt lenBits)
            ...
            modify $ mapOut (concOcWithReversedList bytes)
...
distCodeToBase 0 = 1
distCodeToBase dc = 2^(distCodeToBitNum $ dc - 1) + (distCodeToBase $ dc - 1)
...
parseCompressedBlock codeMap distMap = do
  ...
  case code of _ | code < 0 -> throwUp "Error: Code<0 in parseCompressedBlock"
                 | code <= 255 -> do !o <- gets out
                                     modify $ setOut (mcons (fromIntegral code) o)
...
readCodeLengthsRaw n ct =
  ...
  in
   do
     ...
     case clc of _ | clc <= 15 -> proceed n clc 0 1
...
toplevelParser :: DeflateParse [Word8]
toplevelParser =
  ...
  in do
    ...
    lBlock
    !o <- gets out
    let !ro = reverse (oc2list o) in return ro

inflate :: [Word8] -> Either String [Word8]
inflate !str =
  let !blub = Control.Monad.State.runState (
        runErrorT ( unDeflateParse toplevelParser )) ( DeflateState { strIn = (0, 0, str), out = Nil, inLen = length str })
...
gunzip :: [Word8] -> Either String [Word8]
...
mainWithArgs args = do
  ...
  case gunzip (Data.ByteString.unpack content) of
    ...
    Right x -> do
      print "Success."
      Data.ByteString.writeFile (args !! 1) (Data.ByteString.pack x)
